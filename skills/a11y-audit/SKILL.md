...
- A sitemap group can have no template. Metaobject definitions with storefront URLs enabled list every entry in the sitemap whether or not the theme ships a template for them, and each of those URLs renders the 404 page (`body.template-404`, HTTP 404). `scan.sh` records the final HTTP status and warns on 404; `summary.md` marks the page **HTTP 404**. Never count it as a page sample (it is a second 404-page scan, not a template), drop it from the sample, and put one line in the report's scope: "N sitemap URLs under /<type>/ return 404: the metaobject type has no template."
...
- **Never read the raw pa11y/Lighthouse JSON into context.** Read `summary.md` and `findings.json` only; the merge script dedups cross-engine by (SC, selector), caps sample nodes at 5 per rule per page, and separates violations from the judgment queue. Two things it writes on the side: `contrast-N-<slug>.js` per page holds every deduped contrast selector for step 4d (load it into the browser, don't read it), and `shopify_patterns` in `findings.json` counts the warning/notice noise every theme emits by construction (hidden inputs; the `/localization` country and language forms, rendered six or seven times per page) so it never reaches the judgment queue. One 7-page scan carried over 1,000 of those before the rule existed.
...
- Password-protected store (dev/pre-launch): pa11y `actions` can submit the password form first (`set field #password to X`, `click element [type=submit]`, `wait for path to not be /password`). Unpublished theme: append `?preview_theme_id=<id>` to every URL; note in the report that the audit ran against a preview. Shopify answers that URL with a cookie and a 302 to the bare URL; `scan.sh` and the merge script treat the pair as one page (no redirect warning), and pa11y follows it with cookies, so the preview is what gets measured. The preview bar iframe Shopify injects on every previewed page (`#PBarNextFrame`) is tagged `shopify-preview-bar` and dropped by the merge script as a scan artifact, and `focus_probe.js` skips it the same way; never report it.
- Freshly updated theme (Shopify's theme updater, several major versions at once): audit it via `?preview_theme_id=` before it is published, not after. The updater drops renamed settings and block types silently, so a settled-state contrast failure (text and background both the old `color_background` value), a missing page `h1` (a renamed block type gone), or the vendor's demo navigation in place of the merchant's is usually a settings-migration loss, not a theme defect. Before attributing one of those to the theme, diff the live theme's `settings_data.json` and section groups against the updated copy, and report it as a migration loss with the setting that was dropped.
...
**b. The judgment queue** (`findings.json` → `judgment_queue`): htmlcs warnings/notices are a pre-filtered list of exactly the items worth model judgment, but the htmlcs false-positive rate is high. Triage each rule group: confirm real ones into findings, drop false positives silently, and push genuinely-undecidable ones to the human-check list. `shopify_patterns` is the part already collapsed for you; it leaves one judgment each: does the `/localization` selector submit on change without notice (SC 3.2.2, judge once per theme), and nothing at all for hidden inputs.
...
1. Inject `scripts/contrast_reprobe.js` through the browser driver with the page's whole flagged population, not the 5-per-page `sample_nodes`: the merge script writes it as `contrast-N-<slug>.js` (`window.__a11ySelectors = [...]`), named in `summary.md` and in the group's `reprobe_file`. Load that file first, then the probe (with Playwright, `page.addScriptTag({path})` twice, then read `window.__a11yContrast`; with claude-in-chrome, paste both). With no selectors it auto-samples visible text. It forces animations to their end state, scrolls each target into view, and computes real ratios from composited colors.
2. `at_risk: true` means the page animates: the scanner's contrast count is unusable as printed. Report the re-probed numbers, and say both counts out loud ("axe reported N at load; M fail in the settled state"). The probe's `coverage` block says what M is a count of: `matched_selectors` equal to the group's page count means the whole population was re-measured; anything less, or `truncated_selectors` above zero, and the sentence says "of the K re-measured", never implying the population.
...
4. Text whose color computes to `rgba(0,0,0,0)` is usually knockout or outlined display type that renders fine. Judge it from a screenshot; never report it as invisible text on the strength of the computed value. An `indeterminate` with reason `hidden at probe time` is a scanner hit inside a closed panel, drawer, or menu (visibility:hidden, clipped, offscreen): not a failure on the page at rest. Re-probe with that state open, or leave it Flagged with the state named.
...
1. On each sampled page, inject `scripts/focus_probe.js` via the driver's evaluate (claude-in-chrome `javascript_tool`, Playwright `browser_evaluate`); paste the whole file, it evaluates to one JSON report. A driver that cannot read files (Playwright MCP has no `require`/`fs`): `page.addScriptTag({path})` from `browser_run_code_unsafe`, then evaluate `window.__a11yFocus` (the contrast probe sets `window.__a11yContrast` the same way). On a previewed theme the probe skips `#PBarNextFrame` and reports the skip in `scan_artifacts_skipped`; a preview-bar hit in any list means an old probe. DOM-fact classes (`aria_hidden_focusable`, `invisible_focusable`, `positive_tabindex`, `skip_link`) are Verified; heuristic classes (`no_focus_indicator`, `order_regressions`) are Flagged, per the report's own note. Browser-extension debris can appear in results (coupon extensions inject focusables); discount selectors that clearly aren't the site's.
...
- `scripts/contrast_reprobe.js`: settled-state contrast re-probe for step 4d (injected in-page, zero dependencies; reads `window.__a11ySelectors`, writes `window.__a11yContrast`).
- `scripts/focus_probe.js`: keyboard/focus probe for step 4f (writes `window.__a11yFocus`).
